        ...
        <div className="min-h-screen w-full flex bg-[#f8f6f0] font-body text-[#050B14] absolute inset-0 z-50">
            ...
            <div className="w-full lg:w-[45%] flex flex-col p-8 lg:p-12 xl:p-16 h-full overflow-y-auto">
                ...
                <div className="w-full max-w-md mx-auto grow flex flex-col justify-center">
                    ...
                    <p className="text-gray-500 text-base lg:text-lg mb-8">Sign in to your account</p>

                    {/* Auth Mode Toggle */}
                    <div className="flex gap-1 mb-6 bg-black/5 rounded-full p-1">
                        <button
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {success && (
                            <motion.div
                                ...
                                animate={{ opacity: 1, y: 0 }}
                                exit={{ opacity: 0, y: -5 }}
                                className="bg-emerald-50 text-emerald-600 text-sm p-3 rounded-xl mb-6 text-center border border-emerald-100 font-medium"
                            >
                                {success}
                            </motion.div>
                        )}
                            ...
                            >
                                {!otpSent ? (
                                    <form onSubmit={handleSendOtp} className="space-y-6">
                                        <div>
                                            <label className="block text-gray-400 text-[11px] uppercase tracking-[0.2em] font-bold mb-2.5 pl-4">Phone Number</label>
                                            <div className="relative">
                                                <span className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-400 text-base font-medium">+91</span>
                                                <input
                                                    type="tel"
                                                    value={phone}
                                                    ...
                                                    maxLength={10}
                                                    className="w-full bg-white border border-[#c9a962]/20 shadow-[0_4px_15px_rgba(0,0,0,0.03)] rounded-full py-4 pl-16 pr-6 text-[#050B14] placeholder-gray-300 focus:ring-2 focus:ring-[#c9a962]/40 focus:border-[#c9a962] outline-none transition-all duration-300 text-base font-medium tracking-widest"
                                                    required
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className="pt-2">
                                            <button
                                                ...
                                                className="w-full bg-[#d97736] hover:bg-[#c0612a] text-white font-bold text-lg py-4 rounded-full shadow-[0_8px_20px_rgba(217,119,54,0.3)] hover:shadow-[0_12px_25px_rgba(217,119,54,0.45)] hover:-translate-y-0.5 transition-all duration-300 ease-out flex justify-center items-center gap-2 disabled:opacity-50 disabled:hover:translate-y-0"
                                            >
                                                {loading ? (
                                                    <svg className="animate-spin h-6 w-6 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                                ) : 'Send OTP'}
                                    ...
                                    <form onSubmit={handleVerifyOtp} className="space-y-6">
                                        <div>
                                            <label className="block text-gray-400 text-[11px] uppercase tracking-[0.2em] font-bold mb-2.5 pl-4">Verification Code</label>
                                            <input
                                                type="text"
                                                value={otp}
                                                ...
                                                placeholder="123456"
                                                maxLength={6}
                                                className="w-full bg-white border border-[#c9a962]/20 shadow-[0_4px_15px_rgba(0,0,0,0.03)] rounded-full py-4 px-6 text-[#050B14] placeholder-gray-300 focus:ring-2 focus:ring-[#c9a962]/40 focus:border-[#c9a962] outline-none transition-all duration-300 text-2xl tracking-[0.5em] text-center font-mono font-medium"
                                                required
                                                autoFocus
                                            />
                                        </div>
                                        ...
                                        <div className="pt-2 space-y-3">
                                            ...
                                            <button
                                                ...
                                                className="w-full text-gray-400 hover:text-[#050B14] text-xs tracking-wider uppercase font-bold transition-colors py-2"
                                            >
                                                Change Number
                                            </button>
                                        </div>
                                ...
                                {/* Google sign-in available on phone tab too */}
                                <div className="flex gap-4 mt-8">
                                    <button onClick={handleGoogleLogin} className="flex-1 flex items-center justify-center gap-2 border border-black/10 hover:border-[#c9a962]/50 hover:bg-white bg-transparent py-3.5 rounded-full text-base font-semibold text-[#050B14] shadow-sm hover:shadow-md transition-all duration-300 ease-out hover:-translate-y-0.5">
                                        <svg className="w-5 h-5" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" /><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" /><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" /><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" /></svg>
                                        Google
